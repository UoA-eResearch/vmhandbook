...
If you have an existing standard volume that you want to convert to a Performance Volume, you can do so using the command line interface (CLI) and the `openstack` command. If you are not familiar with the CLI, you can [raise a ticket](http://support.nectar.org.au/) for help, and the Nectar support team can assist you with converting your existing volume to a Performance Volume.
